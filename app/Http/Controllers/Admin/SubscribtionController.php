...
class SubscribtionController extends Controller {
    ...
    */
    ...
    public function postSend(Request $request) {
        if (!$request->has('ids')) {
            return ['status' => 'warning', 'title' => 'error', 'msg' => 'please select at least one email.'];
        }

        if (!$request->has('content')) {
            return ['status' => 'warning', 'title' => 'error', 'msg' => 'message content 
            can not be empty' ];
        }

        if (!$request->has('subject')) {
            return ['status' => 'warning', 'title' => 'error', 
            'msg' => 'message subject can not be empty'];
        }
    ...
    public function postAction($action, Request $r) {
        ...
        if ($r->has('ids')) {
            ...
            $data = ['status' => 'success','title' => 'success', 'msg' => 'process done successfully.'];
        } else {
            $data = ['status' => 'warning', 'title' => 'error','msg' => 'please select at least one email'];
        }
